...
female=female.sort_values(by=['word'])
female=female.drop_duplicates(subset=['word'], keep='first')

female.index=[i for i in range(0,len(female.index))] #renaming the indices
female=female.iloc[0:3281,:]
...
male=male.sort_values(by=['word'])
male=male.drop_duplicates(subset=['word'], keep='first')

male.index=[i for i in range(0,len(male.index))] #renaming the indices
male=male.iloc[0:3804,:]
...
#english words
eng_word = pd.read_csv('five_thousand.csv').head(1000)
...
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [75])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [100])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [125])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [150])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [174])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [197])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
onehotencoder = OneHotEncoder(categorical_features = [218])
df1 = onehotencoder.fit_transform(df1).toarray()
df1 = df1[:, 1:]
...
classifier.add(Dense(output_dim = 6, init = 'uniform', activation = 'relu', input_dim = 237))
